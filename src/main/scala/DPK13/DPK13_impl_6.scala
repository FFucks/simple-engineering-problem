...
class DPK13_impl_6 {
    ...
    def fizzBuzz(numbers: Int): List[Int | String] = {
        ...
        val stack = mutable.Stack[Int | String]()

        for (element <- 1 to numbers) {
            element match {
                case number if number % 15 == 0 => stack.push("FizzBuzz")
                case number if number % 3 == 0  => stack.push("Fizz")
                case number if number % 5 == 0  => stack.push("Buzz")
                case number                     => stack.push(number)
            }
        }
...
@main def dpk13Impl6(): Unit = {

    val dpk13 = new DPK13_impl_6
